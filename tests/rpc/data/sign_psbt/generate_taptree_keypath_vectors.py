...
Generates:
  ...
  psbt_ss_p2tr_taptree_scripts.json   negative: also has a tapleaf script (0x15)
  psbt_ss_p2tr_taptree_multikey.json  negative: more than one taproot keypath

The positive PSBT is derived from psbt_ss_p2tr_default_all.json (same singlesig
mnemonic and m/86'/1'/0'/0/0 internal key) by replacing the spent output with a
single-leaf taptree tweak and adding PSBT_IN_TAP_MERKLE_ROOT (0x18). The two
negative vectors are rebuilt as single-input PSBTs (only the guard-triggering
input) so no signable input remains and sign_psbt returns them unchanged.
...
def main():
    ...
    # Negatives are single-input PSBTs (only the guard-triggering taproot input), so
    # no signable input remains and sign_psbt returns the PSBT unchanged (plan 5.2).
    ...
    # Negative: keypath + merkle root + a tapleaf script (0x15): input is skipped.
    control_block = bytes([TAPLEAF_VERSION]) + internal
    neg_scripts = canonicalize(build_single_input_psbt(
    ...
    write_vector(
        ...
        f'script-path shape, so the input is silently skipped and the PSBT is returned unchanged. {gen}.',
        neg_scripts, {'psbt': neg_scripts})
